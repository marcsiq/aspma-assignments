...
def estimateInharmonicity(inputFile = '../../sounds/piano.wav', t1=0.1, t2=0.5, window='hamming', 
    ...
    hfreq, hmag, hphase = HM.harmonicModelAnal(x, fs, w, N, H, t, nH, minf0, maxf0, f0et, harmDevSlope=0.01, minSineDur=0.0)
    f0 = HM.f0Detection(x, fs, w, N, H, t, minf0, maxf0, f0et)

    # 2. Extract the time segment in which you need to compute the inharmonicity. 
    bint1 = int(np.ceil(t1 * float(fs)/H))
    bint2 = int(np.ceil(t2 * float(fs)/H))
    bhfreq = hfreq[bint1:bint2]
    bf0 = f0[bint1:bint2]
    ...
    for idx, h in enumerate(bhfreq):
        ...
        i = np.abs(h - r * bf0[idx])/r
        inhm = np.append(inhm, np.sum(i) / len(i))
    inharmonicity = np.sum(inhm) / len(inhm)
