...
def segmentStableNotesRegions(inputFile = '../../sounds/sax-phrase-short.wav', stdThsld=10, minNoteDur=0.1, 
    ...
    f0[f0<np.finfo(float).eps] = np.finfo(float).eps   
    f0cent = 1200 * np.log2(f0 / 55.0)    
    
    # 2. create an array containing standard deviation of last winStable samples
    stds = np.zeros(winStable-1)
    for i in range(f0cent.size):
        stds = np.append(stds, np.std(f0cent[i:i+winStable-1]))
    ...
    # 4. create segments of continuous stable points such that concequtive stable points belong to same segment
    seg = []
    if len(stables) > 0:
        sequence = [stables[0]]
        index = 0
        for s in stables:
            if s != (sequence[0] + index):
                seg.append([sequence[0], sequence[len(sequence) - 1]])
                sequence = []
            sequence.append(s)
            index = len(sequence)
    ...
    for s in seg:
        if (s[1] - s[0]) * H / float(fs) >= minNoteDur:
            long_segments.append(s)

    # plotSpectogramF0Segments(x, fs, w, N, H, f0, segments)
    segments = np.array(long_segments)
    plotSpectogramF0Segments(x, fs, w, N, H, f0, segments)
